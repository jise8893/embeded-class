...
static int (*hook_ssl)(SSL *ssl , void *buf , int num)=NULL;
static int (*hook_ssl2)(SSL *ssl, const void *buf,int num)=NULL;
#define MAX 0x100
...
{
	if(hook_ssl ==NULL) hook_ssl =dlsym(RTLD_NEXT,"SSL_read");
	int hook_ret = hook_ssl(ssl,buf,num);

	if (ssl!=NULL&&buf!=NULL&&hook_ret>0)
	{
		Printssl(buf);
	}
	       return hook_ret;
...
{
	if(hook_ssl2 ==NULL) hook_ssl2 =dlsym(RTLD_NEXT,"SSL_write");
	int hook_ret =hook_ssl2(ssl,buf,num);
